...
def pdf_processing_worker(pdf_path, ref_designators, num_threads, highlight_color, result_queue, progress_queue, add_annotation=False, bom_path=None, header_mapping=None):
    ...
    try:
        ...
        progress_queue.put(("warning", error_msg))
...
class BOMCheckerApp:
    def __init__(self, master):
        """初始化应用程序"""
        self.master = master
        self.master.title("BOM Checker v1.4 | 小航  2025.4.1")
    ...
    def __init_default_mapping(self):
        ...
        self.bom_header_mapping = {
            'reference': ['Reference', 'Ref','位号'],
            'pn': ['Part Number', 'PartNumber', 'PN', 'P/N', '料号'],
            'part': ['Type',  'Value', '元件类型', '器件类型'],
            'quantity': ['Quantity', 'Number', '数量', '用量', '用量'],
            'description': ['Description', '描述', '说明', '规格'],
            'mfr_pn': ['ManuFacturer P/N', '制造商零件编号', '厂商料号', '厂商型号'],
            'manufacturer': ['Manufacturer', 'Vendor Name', '制造商名称', '厂商名称']
        }
    ...
    def create_widgets(self):
        ...
        # 帮助按钮
        self.help_button = ttk.Button(header_frame, text="使用帮助", 
                                  command=self.show_help)
        self.help_button.pack(side='right', padx=5)
        
        # 关于按钮
        self.about_button = ttk.Button(header_frame, text="关于", 
...
检查完成后，将生成标记了所有BOM位号的PDF文件。""", 'info')
    ...
    def show_help(self):
        ...
        # 版本信息
        ttk.Label(header_frame, text="V1.4", 
...
🚀 界面优化
• 检查选项独立化：
  ✓ 将"检查选项"区域移至单独的"设置"窗口
  ✓ 优化界面布局，减少主界面占用空间

• PDF操作区优化：
  ✓ 新增"打开"和"打开文件夹"按钮
  ✓ 按钮位置优化，放置于"检查结果"标题右侧
...
"""
    ...
    def parse_bom(self):
        """解析BOM文件，提取位号并处理重复和异常"""
        try:
            # 检测真正的表头行，处理包含项目信息的BOM
            detected_header_row = self._detect_header_row(self.bom_path)
            if detected_header_row > 0:
                # 注释下面的消息输出，不在过程中显示
                # self.message_queue.put(("text", f"检测到表头在第 {detected_header_row+1} 行，已自动跳过项目信息行\n", False))
                pass
            
            # 读取Excel文件，跳过前面的项目信息行，强制将数值列转换为字符串
            df = pd.read_excel(self.bom_path, engine='openpyxl', header=detected_header_row, dtype=str)
            ...
            # 获取位号列可能的名称
            ref_col_names = self.bom_header_mapping['reference']
            
            # 步骤1: 尝试自动匹配位号列
            ref_col = None
            for col in df.columns:
                col_str = str(col).strip()
                if any(ref_name.lower() == col_str.lower() for ref_name in ref_col_names):
                    ref_col = col
                    # 注释下面的消息输出，不在过程中显示
                    # self.message_queue.put(("text", f"自动识别到位号列: {col}\n", False))
                    break
            
            # 步骤2: 如果无法自动匹配，则尝试部分匹配
            if ref_col is None:
                for col in df.columns:
                    col_str = str(col).strip()
                    if any(ref_name.lower() in col_str.lower() for ref_name in ref_col_names):
                        ref_col = col
                        # 注释下面的消息输出，不在过程中显示
                        # self.message_queue.put(("text", f"通过部分匹配识别到位号列: {col}\n", False))
                        break
            
            # 步骤3: 如果仍无法匹配，则弹出对话框让用户选择并更新映射
            if ref_col is None:
                # 将列名转换为字符串列表
                column_names = [str(col) for col in df.columns]
                ref_col, should_remember = self.ask_user_for_column(column_names)
                
                if ref_col is None:  # 用户取消了选择
                    return []
                    
                # 记住用户的选择并更新映射
                if should_remember and ref_col:
                    if 'reference' in self.bom_header_mapping:
                        if ref_col not in self.bom_header_mapping['reference']:
                            self.bom_header_mapping['reference'].append(ref_col)
                    else:
                        self.bom_header_mapping['reference'] = [ref_col]
                    # 保存更新后的映射
                    self.save_header_mapping()
                    self.message_queue.put(("text", f"已将 '{ref_col}' 添加到位号列映射\n", False))
            
            # 尝试识别其他必要列
            item_col = self._identify_column(df, 'item', '序号列')
            pn_col = self._identify_column(df, 'pn', '料号列')
            quantity_col = self._identify_column(df, 'quantity', '数量列')
            mfr_pn_col = self._identify_column(df, 'mfr_pn', '厂商料号列')
            
            # 创建一个新的DataFrame来存储完整的物料信息
            bom_data = pd.DataFrame()
            
            # 将识别到的列添加到bom_data
            if ref_col:
                bom_data['Reference'] = df[ref_col].astype(str).replace('nan', '')
            if item_col:
                bom_data['Item'] = df[item_col].astype(str).replace('nan', '')
            if pn_col:
                bom_data['PN'] = df[pn_col].astype(str).replace('nan', '')
            if quantity_col:
                bom_data['Quantity'] = df[quantity_col]
            if mfr_pn_col:
                bom_data['MfrPN'] = df[mfr_pn_col].astype(str).replace('nan', '')
            
            # 验证BOM数据
            validation_results, processed_refs = self._validate_bom_data(bom_data)
            ...
            error_msg = f"解析BOM文件时出错: {str(e)}"
            self.message_queue.put(("text", error_msg + "\n", False))
            self.message_queue.put(("status", "BOM解析失败", self.error_color))
            print(error_msg)
            traceback.print_exc()  # 打印详细错误信息
            return []
    
    def _identify_column(self, df, field_key, field_desc):
        """识别指定类型的列"""
        field_names = self.bom_header_mapping.get(field_key, [])
        
        # 精确匹配
        for col in df.columns:
            col_str = str(col).strip()
            if any(name.lower() == col_str.lower() for name in field_names):
                # 注释下面的消息输出，不在过程中显示
                # self.message_queue.put(("text", f"自动识别到{field_desc}: {col}\n", False))
                return col
        
        # 部分匹配
        for col in df.columns:
            col_str = str(col).strip()
            if any(name.lower() in col_str.lower() for name in field_names):
                # 注释下面的消息输出，不在过程中显示
                # self.message_queue.put(("text", f"通过部分匹配识别到{field_desc}: {col}\n", False))
                return col
    ...
    def _validate_bom_data(self, bom_data):
        ...
        if 'Item' in bom_data.columns:
            ...
            for _, row in bom_data.iterrows():
                ...
                try:
                    ...
                    if base_item:
                        if base_item not in alternative_groups:
                            alternative_groups[base_item] = []
                        alternative_groups[base_item].append(item)
                except:
                    # 如果拆分失败，跳过
                    continue
        
        # 记录有效的替代料关系（至少有两个物料）
        for base_item, items in list(alternative_groups.items()):
            if len(items) > 1:
                validation_results['alternative_parts'][base_item] = items
            else:
                # 如果只有一个物料，不是有效的替代料组
                del alternative_groups[base_item]
        ...
        if 'MfrPN' in bom_data.columns:
            ...
            valid_mfrpns = bom_data[bom_data['MfrPN'].notna() & (bom_data['MfrPN'].str.strip() != '')]['MfrPN'].str.strip()
            mfrpn_counts = valid_mfrpns.value_counts()
            duplicate_mfrpns = [mfrpn for mfrpn, count in mfrpn_counts.items() if count > 1 and mfrpn]
            
            if duplicate_mfrpns:
                validation_results['duplicate_mfrpns'] = duplicate_mfrpns
        ...
        if 'Reference' in bom_data.columns:
            for idx, row in bom_data.iterrows():
                ...
                # 如果位号或料号为空，跳过
                if not refs_str or not pn or refs_str.lower() == 'nan' or pn.lower() == 'nan':
                    continue
                
                # 拆分位号，支持不同分隔符
                refs = [ref.strip() for ref in re.split(r'[,;\s]+', refs_str) if ref.strip() and ref.strip().lower() != 'nan']
            ...
            for ref, pns in ref_to_pns_map.items():
                # 如果位号只在一个物料中出现，跳过（没有重复）
                if len(pns) <= 1:
                    continue
                
                # 添加调试输出
                print(f"检查位号 {ref} 在多个物料中重复: {pns}")
                
                # 改进的替代料组检测逻辑
                # 策略1: 基于Item格式的标准替代料检测
                items_list = []
                for pn in pns:
                    if pn in pn_to_item_map:
                        item_val = pn_to_item_map[pn]
                        items_list.append(item_val)
                        print(f"  物料 {pn} 的Item编号: {item_val}")
                    else:
                        # 如果找不到物料的Item，添加一个占位值
                        items_list.append("")
                        print(f"  物料 {pn} 没有找到Item编号")
                
                # 策略1: 基于Item的标准检测
                # 检查是否有任何物料拥有替代料格式的Item，如果有，则提取并比较基础编号
                items_with_format = [item for item in items_list if item and '.' in item]
                if items_with_format:
                    ...
                    try:
                        ...
                        for item in items_with_format:
                            ...
                            print(f"  提取基础编号: {item} → {base_item}")
                        
                        # 如果所有有格式的Item都有相同的基础编号，认为是替代料组
                        if len(base_items) == 1:
                            print(f"  基于Item格式判定为替代料组，跳过")
                            continue
                        print(f"  基础编号不同: {base_items}")
                    except Exception as e:
                        print(f"  基础编号提取出错: {str(e)}")
                
                # 策略2: 查找替代料组字典中的关系
                # 检查是否这些物料已经被归类到替代料组中
                found_in_alt_group = False
                
                # 首先构建一个PN到基础Item的映射，用于快速查找
                pn_to_base_item = {}
                for base_item, alt_items in alternative_groups.items():
                    for alt_item in alt_items:
                        ...
                        for idx, row in bom_data.iterrows():
                            ...
                            pn_val = str(row.get('PN', '')).strip()
                            if item_val == alt_item and pn_val:
                                pn_to_base_item[pn_val] = base_item
                
                # 检查这些物料是否都属于同一个替代料组
                base_groups = set()
                for pn in pns:
                    if pn in pn_to_base_item:
                        base_groups.add(pn_to_base_item[pn])
                
                if len(base_groups) == 1:
                    print(f"  通过替代料组字典判定为替代料组: {base_groups}，跳过")
                    continue
                elif base_groups:
                    print(f"  物料属于不同的替代料组: {base_groups}")
                
                # 策略3: 启发式判断 - 检查物料编号是否有关联
                # 例如：如果物料编号相似（仅有少量字符不同），可能是替代料
                if len(pns) == 2:  # 暂时只对两个物料的情况使用这种启发式方法
                    pn1, pn2 = pns
                    # 如果两个物料编号长度相同且有较高的相似度，可能是替代料
                    if len(pn1) == len(pn2):
                        # 计算不同字符的数量
                        diff_count = sum(c1 != c2 for c1, c2 in zip(pn1, pn2))
                        # 如果不同字符少于等于3个，认为是替代料
                        if diff_count <= 3:
                            print(f"  基于物料编号相似度判定为替代料组: {pn1} vs {pn2}，不同字符数: {diff_count}，跳过")
                            continue
                
                # 策略3: 启发式判断 - 检查物料编号是否有关联(支持多个物料)
                # 这种策略检查一组物料是否可能是替代料，基于它们料号的相似性
                if len(pns) >= 2:
                    print(f"  尝试使用启发式方法分析多个物料: {pns}")
                    
                    # 方法A: 检查所有物料长度是否相同，且物料编号相似
                    lengths = set(len(pn) for pn in pns)
                    if len(lengths) == 1:  # 所有物料编号长度相同
                        length = next(iter(lengths))
                        
                        # 只在物料编号足够长时进行相似度检查，避免短编号的误判
                        if length >= 8:  # 假设物料编号至少有8个字符
                            print(f"  所有物料编号长度相同({length})，进行相似度分析")
                            
                            # 检查是否所有物料编号只有少量不同字符
                            # 构建一个字符频率矩阵，找出哪些位置的字符是一致的
                            char_matrix = []
                            for i in range(length):
                                char_set = set(pn[i] for pn in pns)
                                char_matrix.append(char_set)
                            
                            # 计算有多少位置的字符完全一致，以及有多少位置的字符不同
                            # 如果不同的位置较少，可能是替代料
                            consistent_positions = sum(1 for chars in char_matrix if len(chars) == 1)
                            diff_positions = length - consistent_positions
                            
                            print(f"  字符一致的位置数: {consistent_positions}, 不同的位置数: {diff_positions}")
                            
                            # 如果不同位置少于等于总长度的30%，且不超过5个位置，则认为是替代料组
                            if diff_positions <= min(5, length * 0.3):
                                print(f"  基于全局相似度判定为替代料组，不同位置: {diff_positions}，跳过")
                                continue
                    
                    # 方法B: 检查是否所有物料都与组内某一物料有高相似度
                    # 构建一个相似度图，检查是否所有物料都至少与一个其他物料有较高的相似度
                    similar_pairs = []
                    for i in range(len(pns)):
                        for j in range(i+1, len(pns)):
                            ...
                            # 只比较长度相等的物料编号
                            if len(pn1) == len(pn2) and len(pn1) >= 8:
                                # 计算不同字符的数量
                                diff_count = sum(c1 != c2 for c1, c2 in zip(pn1, pn2))
                                # 如果不同字符少于等于3个，认为这两个物料相似
                                if diff_count <= 3:
                                    similar_pairs.append((pn1, pn2))
                                    print(f"  物料对相似: {pn1} vs {pn2}, 差异: {diff_count}字符")
                    ...
                    for pn1, pn2 in similar_pairs:
                        similarity_graph[pn1].append(pn2)
                        similarity_graph[pn2].append(pn1)
                    
                    # 检查图的连通性 - 如果所有节点都可以互相到达，则说明它们都属于同一组
                    # 使用BFS算法检查连通性
                    visited = set()
                    def bfs(start):
                        queue = [start]
                    ...
                    # 如果所有物料都被访问到，说明图是连通的，所有物料都是相互关联的
                    if len(visited) == len(pns):
                        print(f"  基于物料相似度网络判定为替代料组，所有物料都是相互关联的，跳过")
                        continue
                    elif len(visited) > 0:
                        print(f"  物料相似度网络不完全连通，访问到 {len(visited)}/{len(pns)} 个物料")
                
                # 方法C: 检查前缀/后缀相似性
                # 有些替代料可能有相同的前缀或后缀，但中间部分不同
                if len(pns) >= 2:
                    min_length = min(len(pn) for pn in pns)
                    if min_length >= 8:  # 确保物料编号足够长
                        # 检查前缀相似性 (前6个字符)
                        prefix_length = min(6, min_length - 2)  # 至少留2个字符作为变化部分
                        prefixes = set(pn[:prefix_length] for pn in pns)
                        
                        # 检查后缀相似性 (后4个字符)
                        suffix_length = min(4, min_length - 4)  # 至少留4个字符作为前缀和变化部分
                        suffixes = set(pn[-suffix_length:] for pn in pns)
                        
                        print(f"  前缀集合({prefix_length}字符): {prefixes}")
                        print(f"  后缀集合({suffix_length}字符): {suffixes}")
                        
                        # 如果所有物料有相同的前缀或后缀，则可能是替代料
                        if len(prefixes) == 1 or len(suffixes) == 1:
                            print(f"  基于前缀/后缀相似性判定为替代料组，跳过")
                            continue
                
                # 经过以上所有策略判断，如果仍未被确定为替代料组，则添加到冲突列表
                validation_results['duplicate_refs'].append({
                    'ref': ref,
                    'pns': pns,
                    'same_part': False  # 不同物料间重复
                })
                print(f"  添加到冲突列表：所有替代料组检测策略均失败")
    ...
    def _display_validation_results(self, validation_results):
        ...
        if validation_results['duplicate_pns']:
            self.message_queue.put(("text", f"警告: 发现 {len(validation_results['duplicate_pns'])} 个重复料号\n", False))
        
        # 显示重复厂商料号警告
        if validation_results.get('duplicate_mfrpns'):
            self.message_queue.put(("text", f"警告: 发现 {len(validation_results['duplicate_mfrpns'])} 个重复厂商料号\n", False))
        
        # 显示重复位号警告
        if validation_results['duplicate_refs']:
            self.message_queue.put(("text", f"警告: 发现 {len(validation_results['duplicate_refs'])} 个重复位号\n", False))
        ...
        if hasattr(self, 'validation_results'):
            ...
            if self.validation_results.get('duplicate_refs'):
                ...
                for ref_info in self.validation_results['duplicate_refs']:
                    if ref_info.get('same_part', False):
                ...
                if diff_part_dups:
                    ...
                    for ref_info in diff_part_dups[:3]:  # 最多显示3个
                        ref = ref_info['ref']
                        pns = ref_info['pns']
                        self._update_result_text(f"    - {ref} 出现在: {' 和 '.join(pns)}\n", False, 'info')
                    if len(diff_part_dups) > 3:
                        self._update_result_text(f"    - ... 等 {len(diff_part_dups)-3} 个\n", False, 'info')
            ...
            if self.validation_results.get('quantity_mismatch'):
                ...
                # 显示所有不匹配信息
                for i, mismatch in enumerate(self.validation_results['quantity_mismatch']):
                    if i < 5:  # 最多显示5个
                        self._update_result_text(f"   - {mismatch['item']} {mismatch['pn']}: BOM中数量 {mismatch['expected']}, 实际位号数 {mismatch['actual']}\n", False, 'info')
                if len(self.validation_results['quantity_mismatch']) > 5:
                    self._update_result_text(f"   - ... 等 {len(self.validation_results['quantity_mismatch'])-5} 个\n", False, 'info')
    ...
    def show_about(self):
        ...
        version_label = ttk.Label(title_info, text="            V1.4", 
                              font=('微软雅黑', 12, 'bold'))
    ...
    def _detect_header_row(self, excel_path):
        ...
        try:
            # 读取Excel文件的前10行进行分析
            max_rows_to_check = 15
            df_preview = pd.read_excel(excel_path, engine='openpyxl', nrows=max_rows_to_check, header=None)
            
            # 获取所有可能的表头关键词
            header_keywords = []
            for field, values in self.bom_header_mapping.items():
                header_keywords.extend(values)
            ...
            for row_idx in range(min(max_rows_to_check, len(df_preview))):
                ...
                # 计算行得分：非空单元格比例 + 关键词匹配率
                if non_empty_cells > 0:
                    # 行的得分 = 关键词匹配数 * 2 + 非空单元格数量
            ...
            # 备选策略：查找第一个包含参考列头关键词的行
            for row_idx in range(min(max_rows_to_check, len(df_preview))):
                row = df_preview.iloc[row_idx]
                for cell in row:
                    if pd.notna(cell):
                        cell_str = str(cell).strip().lower()
                        ref_keywords = [kw.lower() for kw in self.bom_header_mapping['reference']]
                        if any(kw == cell_str or kw in cell_str for kw in ref_keywords):
                            return row_idx
    ...
    def ask_user_for_column(self, column_names):
        """弹出对话框，让用户选择位号列并选择是否记住选择"""
        # 创建对话框窗口但不立即显示
        ...
        ttk.Label(frame, text="未能自动识别位号列，请手动选择:", 
                font=('微软雅黑', 10, 'bold')).pack(pady=(0, 5))
        
        ttk.Label(frame, text="(位号列通常包含如 R1, C2, U3 等器件编号)", 
