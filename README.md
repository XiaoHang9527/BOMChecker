...
**V1.6 (2024.4)**
- 所有错误信息中文化显示，提升用户体验
- 修复BOM表头映射保存问题，增强用户自定义表头的稳定性
- 增强异常处理机制，提供更直观的错误提示
- 添加chardet模块缺失时的备用编码检测机制

**V1.5 (2024.4)**
- 增强表头识别功能，支持更多格式的BOM文件
- 优化用户界面交互，改进表头映射编辑器
- 增强文件编码自动识别功能
- 改进配置文件管理，增强用户设置持久化

**V1.4 (2024.4)**
- 修复纯数字料号处理问题，增强数据类型转换
- 增强空值处理，避免"nan"显示为位号数量或重复位号
- 优化Excel读取逻辑，增强对不同格式的兼容性
- 改进错误处理机制，提供更详细的错误信息

**V1.3 (2024.3)**
- 添加了仅BOM检查模式，不需要PDF文件也可以验证BOM数据
- 优化了替代料识别算法，增加了多种识别策略
- 改进了UI界面，添加了颜色预览功能
- 增强了多进程架构稳定性，通过队列优化了进程间通信
- 添加了统一配置文件管理，保存用户所有设置
- 优化了内存使用，添加了Windows进程优先级设置

**V1.2 (2024.3)**
- 增加了BOM数据验证功能，支持检查重复料号和位号
- 增加了替代料关系的识别和处理
- 增加了位号数量核对功能
- 增加了物料信息注解功能，支持在PDF中查看完整物料信息
- 优化了多进程架构，提升了界面响应速度

**V1.1 (2024.3)**
- 增加了自动识别表头功能
- 增加了多线程支持，加快处理速度
- 优化了内存使用，提升了大型PDF处理的稳定性
